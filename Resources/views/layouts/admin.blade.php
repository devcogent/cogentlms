...
<head>
    ...
    <link rel="stylesheet"
        href="{{ asset('client/themes/admin/assets/modules/weather-icon/css/weather-icons.min.css') }}">
    <link rel="stylesheet"
        href="{{ asset('client/themes/admin/assets/modules/weather-icon/css/weather-icons-wind.min.css') }}">
    <link rel="stylesheet" href="{{ asset('client/themes/admin/assets/modules/summernote/summernote-bs4.css') }}">
    <link type="text/css" rel="stylesheet"
        href="{{ asset('client/vendor/jquery-ui/css/ui-lightness/jquery-ui-1.10.4.css') }}" />
        ...
        <div class="main-wrapper main-wrapper-1" id="content">
                ...
                <ul class="navbar-nav navbar-right">
                    ...
                    <li class="dropdown"><a href="#" data-toggle="dropdown"
                            ...
                            @if (!empty(Auth::user()->picture) && file_exists(Auth::user()->picture))
                                <img alt="image" src="{{ asset(Auth::user()->picture) }}"
                                    class="rounded-circle mr-1">
                            @else
                                <img alt="image"
                                    src="{{ asset('client/themes/admin/assets/img/avatar/avatar-1.png') }}"
                                    class="rounded-circle mr-1">
                            @endif
                            <div class="d-sm-none d-lg-inline-block">@lang('default.hi'), {{ Auth()->user()->name }}
                            </div>
                        </a>
            ...
            <div class="main-sidebar sidebar-style-2">
                <aside id="sidebar-wrapper">
                    ...
                    <ul class="sidebar-menu">
                        ...
                        <li><a href="@route('admin.dashboard')" class="nav-link"><i
                                    class="fas fa-fire"></i><span>@lang('default.dashboard')</span></a></li>

                        @can('access-group', 'course')
                            <li class="dropdown">
                                <a href="#" class="nav-link has-dropdown"><i
                                        class="fas fa-book"></i><span>{{ __lang('courses-sessions') }}</span></a>
                                <ul class="dropdown-menu">

                                    <li class="dropdown">
                                        <a href="#"
                                            class="nav-link has-dropdown"><span>{{ __lang('add-new') }}</span></a>
                                        <ul class="dropdown-menu">
                                            @can('access', 'add_course')
                                                <li><a class="nav-link"
                                                        href="@route('admin.session.addcourse')">{{ __lang('online-course') }}</a></li>
                                            @endcan
                                    ...
                                    <li><a class="nav-link"
                                            href="{{ route('admin.student.sessions') }}">{{ __lang('all-courses-session') }}
                                        </a></li>
                            ...
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    @can('access', 'add_student')
                                        <li><a class="nav-link" href="@route('admin.student.add')">{{ __('Add-Trainee') }}</a></li>
                                    @endcan
                                    @can('access', 'view_students')
                                        <li><a class="nav-link" href="@route('admin.student.index')">{{ __('All-Trainee') }}</a></li>
                                    @endcan
                        ...
                        @can('access-group', 'classes')
                            <li class="dropdown">
                                <a href="#" class="nav-link has-dropdown"><i
                                        class="fas fa-chalkboard-teacher "></i><span>{{ __lang('classes') }}</span></a>
                                <ul class="dropdown-menu">
                                    @can('access', 'add_class')
                                        <li><a class="nav-link" href="@route('admin.lesson.add')">{{ __lang('add-class') }}</a></li>
                                    @endcan
                                    @can('access', 'view_classes')
                                        <li><a class="nav-link" href="@route('admin.lesson.index')">{{ __lang('all-classes') }}</a>
                                        </li>
                                    @endcan
                        ...
                        <ul class="dropdown-menu">
                            @can('access', 'add_note')
                                <li ><a class="nav-link" href="@route('admin.homework.add')">{{ __lang('add-note') }}</a></li>
                            @endcan
                            @can('access', 'view_notes')
                                <li ><a class="nav-link" href="@route('admin.homework.index')">{{ __lang('view-archive') }}</a></li>
                            @endcan
                    ...
                    <li class="dropdown">
                        <a href="#" class="nav-link has-dropdown"><i class="fas fa-download"></i><span>{{ __('Create Resources') }}</span></a>
                        <ul class="dropdown-menu">
                            @can('access', 'add_download')<li ><a class="nav-link" href="@route('admin.download.add')">{{ __('Create New') }}</a></li>@endcan
                            @can('access', 'view_downloads')<li ><a class="nav-link" href="@route('admin.download.index')">{{ __('List Of Resources') }}</a></li>@endcan
                        </ul>
                    </li>
                    @endcan  --}}
                        ...
                        <ul class="dropdown-menu">
                            @can('access', 'view_discussions')<li ><a class="nav-link" href="@route('admin.discuss.index')">{{ __('Chat') }}</a></li>@endcan
                        ...
                        @can('access-group', 'tests')
                            <li class="dropdown">
                                <a href="#" class="nav-link has-dropdown"><i
                                        class="fas fa-check-circle"></i><span>{{ __lang('tests') }}</span></a>
                                <ul class="dropdown-menu">
                                    @can('access', 'view_tests')
                                        <li><a class="nav-link" href="@route('admin.test.add')">{{ __lang('add-test') }}</a></li>
                                    @endcan
                                    @can('access', 'add_test')
                                        <li><a class="nav-link" href="@route('admin.test.index')">{{ __lang('all-tests') }}</a></li>
                                    @endcan
                                </ul>
                            </li>
                        @endcan
                        ...
                        @can('access-group', 'settings')
                            <li class="dropdown">
                                <a href="#" class="nav-link has-dropdown"><i
                                        class="fas fa-cogs"></i><span>{{ __lang('settings') }}</span></a>
                                <ul class="dropdown-menu">
                                    @can('access', 'edit_site_settings')
                                        <li><a class="nav-link" href="@route('admin.setting.index')">{{ __lang('site-settings') }}</a>
                                        </li>
                                    @endcan
                               ...
                               @can('access', 'edit_site_settings')<li ><a class="nav-link" href="@route('admin.setting.language')">{{ __lang('language') }}</a></li>@endcan
                                    ...
                                    @can('access', 'configure_sms_gateways')
                                        {{--  <li ><a class="nav-link" href="@route('admin.smsgateway.index')">{{ __lang('sms-setup') }}</a></li>  --}}
                                    @endcan

                                    @can('access', 'view_roles')
                                        <li><a class="nav-link" href="@route('admin.roles.index')">{{ __lang('roles') }}</a></li>
                                    @endcan
                                    @can('access', 'view_admins')
                                        <li><a class="nav-link"
                                                href="@route('admin.admins.index')">{{ __('Administrators/Trainer') }}</a></li>
                                    @endcan
                                    @can('access', 'view_test_grades')
                                        <li><a class="nav-link" href="@route('admin.setting.testgrades')">{{ __lang('grades') }}</a></li>
                                    @endcan
            ...
            <footer class="main-footer">
                <div class="footer-left">
                    {{ __lang('copyright') }} &copy; {{ date('Y') }} <a
                        href="{{ config('app.author_url') }}">{{ config('app.app_author') }}</a>
                </div>
