    ...
    <div id="app">
        <div class="main-wrapper container">
            ...
            <nav class="navbar navbar-expand-lg main-navbar">
                <a href="{{ url('/') }}" class="navbar-brand sidebar-gone-hide"><img style="width: 10%;"
                ...
                <ul class="navbar-nav navbar-right">
                    @if (getCart()->hasItems())
                        <li>
                            <a href="{{ route('cart') }}" class="nav-link nav-link-lg  beep"><i
                                    class="fa fa-cart-plus"></i> {{ getCart()->getTotalItems() }}</a>
                        </li>
                    @endif
                    <li class="dropdown"><a href="#" data-toggle="dropdown"
                            class="nav-link dropdown-toggle nav-link-lg nav-link-user">
                            @if (!empty(Auth::user()->picture) && file_exists(Auth::user()->picture))
                                <img alt="image" src="{{ resizeImage(Auth::user()->picture, 50, 50, url('/')) }}"
                            ...
                            @else
                                <img alt="image"
                                    ...
                                    class="rounded-circle mr-1">
                            @endif
                            <div class="d-sm-none d-lg-inline-block">@lang('default.hi'),
                                {{ limitLength(Auth()->user()->name, 30) }}</div>
                        </a>
                ...
                <div class="container">
                    <ul class="navbar-nav">
                        ...
                        <li class="nav-item">
                            <a href="{{ route('student.dashboard') }}" class="nav-link"><i
                                    class="fas fa-fire"></i><span>{{ __lang('dashboard') }}</span></a>
                        </li>
                        <li class="nav-item dropdown">
                            <a href="#" data-toggle="dropdown" class="nav-link has-dropdown"><i
                                    class="fas fa-chalkboard-teacher"></i><span>{{ setting('label_my_sessions', __lang('my-courses')) }}</span></a>
                            <ul class="dropdown-menu dropdown-border">
                                <li class="nav-item"><a href="{{ route('student.student.mysessions') }}"
                                        class="nav-link">{{ __lang('enrolled-courses') }}</a></li>
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-border">
                            ...
                            <li class="nav-item"><a href="{{ route('student.download.index') }}" class="nav-link">{{ __('Resources Download') }}</a></li>
                            @endif
                        </ul>
                    </li>
                    @endif    --}}
                        ...
                        <ul class="dropdown-menu dropdown-border">
                            <li class="nav-item"><a href="{{ route('student.student.discussion') }}" class="nav-link">{{ __('Trainer-Chat') }}</a></li>
                        ...
                        @if (setting('menu_show_tests') == 1)
                            <li class="nav-item dropdown">
                                <a href="#" data-toggle="dropdown" class="nav-link has-dropdown"><i
                                        class="fas fa-check-circle"></i><span>{{ setting('label_take_test', __lang('tests')) }}</span></a>
                                <ul class="dropdown-menu dropdown-border">
                                    <li class="nav-item"><a href="{{ route('student.test.index') }}"
                                            class="nav-link">{{ __lang('browse-tests') }}</a></li>
                                    <li class="nav-item"><a href="{{ route('student.test.statement') }}"
                                            class="nav-link">{{ __lang('statement-of-result') }}</a></li>
                                </ul>
                            </li>
                        @endif
                    </ul>
