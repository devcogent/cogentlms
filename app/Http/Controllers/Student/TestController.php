...
class TestController extends Controller {
    ...
    public function taketest(Request $request,$id)
    {
        ...
        if(!empty($testRow->private) && !isset($courseTest[$id])){
            ...
            foreach($rowset as $row){
                if((Carbon::parse($row->opening_date)->format('Y-m-d') <= Carbon::parse(Carbon::now())->format('Y-m-d')))
                {
                  $opened=true;
                }
            ...
            foreach($rowset as $row){
                if(Carbon::parse($row->closing_date)->format('Y-m-d') >= Carbon::parse(Carbon::now())->format('Y-m-d'))
                {
                   $closed = true;
                }
    ...
    public function statement(Request $request)
    {
        ...
        $attempt =  DB::table('student_tests')->join('course_test', 'course_test.test_id', 'student_tests.test_id')
                                  ->where('student_tests.student_id',$id)->whereNotNull('score')->exists();
